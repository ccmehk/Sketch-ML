...
for image_path in image_paths:
    ...
    if len(kp) is not 0:
    #kpts, des = sift.compute(im, detector)
    #kpts, des = des_ext.compute(im, kpts)
        des_list.append((image_path, des))
        tmpdes = des
    else:
        des_list.append((image_path, tmpdes))
        print 1  
...
for src, i in zip(image_paths, clf.predict_proba(test_features)):
    sort= np.argsort(i)[-5:]
    print src
    print list(reversed([classes_names[j] for j in sort]))
#    print np.argsort(i)
#    for x in i:
#        for j in np.argsort(x)[-3:]:
#            print classes_names[j]
...
#for image_path, prediction in zip(image_paths, predictions):
 #   print image_path
#    print prediction
#    head, tail = os.path.split(os.path.split(image_path)[0])
 #   if tail == prediction:
 #     correct = correct +1
#    total = total + 1
